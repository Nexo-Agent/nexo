...
import { shouldShowControls } from './control-utils';
...
const MemoOl = memo<OlProps>(
  ({ children, className, node, ...props }: OlProps) => (
    <ol
      className={cn(
        'list-inside list-decimal whitespace-normal in-[li]:pl-6',
        className
      )}
...
const MemoUl = memo<UlProps>(
  ({ children, className, node, ...props }: UlProps) => (
    <ul
      className={cn(
        'list-inside list-disc whitespace-normal in-[li]:pl-6',
        className
      )}
  ...
  ({ children, className, node, ...props }: ThProps) => (
    <th
      className={cn(
        ...
        'min-w-[100px]', // Minimum width for better readability
        className
      )}
...
const MemoTd = memo<TdProps>(
  ({ children, className, node, ...props }: TdProps) => (
    <td
      className={cn(
        'px-4 py-2 text-sm',
        'min-w-[100px] max-w-[400px]', // Min width for readability, max width to prevent excessive width
        'break-words', // Allow word breaking for long content
        className
      )}
      ...
      const isEmptyFootnote = (listItem: React.ReactNode): boolean => {
        ...
        for (const itemChild of itemChildren) {
          ...
          if (typeof itemChild === 'string') {
            ...
            if (itemChildProps['data-footnote-backref'] !== undefined) {
              hasBackref = true;
            } else {
...
const MemoParagraph = memo<ParagraphProps>(
  ({ children, className, node, ...props }: ParagraphProps) => {
    ...
    if (validChildren.length === 1 && isValidElement(validChildren[0])) {
      const childProps = validChildren[0].props as {
        node?: MarkdownNode & { tagName?: string };
      };
      if (childProps.node?.tagName === 'img') {
        return <>{children}</>;
      }
    }
